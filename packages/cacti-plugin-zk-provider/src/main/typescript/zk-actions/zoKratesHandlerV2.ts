...
export class ZeroKnowledgeHandlerV2 {
    ...
    try {
      ...
      if (circuitCompilation) {
        ...
        const zkSnark = await new Promise<Proof>((resolve, reject) => {
          ...
          worker.on("message", (result) => resolve(result.proof as Proof));
          worker.on("error", reject);
          worker.on("exit", (code) => {
            if (code !== 0)
              reject(new Error(`Worker stopped with exit code ${code}`));
  ...
  public async verifyProof(proof: any, vk: VerificationKey): Promise<boolean> {
    ...
    try {
      ...
      //const provider1 = await initialize();
      //const validity = await provider1.verify(vk, proof);
      return validity;
    } catch (error) {
      throw new ZoKratesComputationError(error.message, fntag);
    }
