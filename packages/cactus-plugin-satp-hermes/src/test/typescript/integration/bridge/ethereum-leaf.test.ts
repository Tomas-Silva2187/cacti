...
describe("Ethereum Leaf Test", () => {
  ...
  it("Should fetch the contract bytecode, which should be the same as the one in the respective ontology", async () => {
    ...
    const response = await ethereumLeaf.getContractBytecode(wrapperContractAddress);
    expect(response).toBeDefined();

    console.log(response.response);
    const response2 = await ontologyManager.checkOntologyBytecode(LedgerType.Ethereum, "SATP-ERC20-ETHEREUM", ethereumLeaf);
